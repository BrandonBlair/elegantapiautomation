...
def test_can_login(logged_in_session):
    resp = logged_in_session.get(url="http://127.0.0.1:5000/search")
    assert 'Search By ISBN-10' in resp.text

def test_can_skip_login_using_browser(logged_in_session, browser):
    ...
    browser.add_cookie({'name': 'LSESSION', 'value': lsession})
